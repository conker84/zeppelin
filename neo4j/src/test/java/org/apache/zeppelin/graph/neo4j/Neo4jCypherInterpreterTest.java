...
public class Neo4jCypherInterpreterTest {
  ...
  public void testMultiQueries() {
    InterpreterResult result = interpreter.interpret("CREATE (n:Node{name: ';'}); RETURN 1;", context);
    assertEquals(Code.SUCCESS, result.code());
    System.out.println("result.toString() = " + result.toString());
    result = interpreter.interpret("CREATE (n:Node{name: \";\"}); RETURN 2;", context);
    assertEquals(Code.SUCCESS, result.code());
    System.out.println("result.toString() = " + result.toString());
    result = interpreter.interpret("match (n:Node{name: ';'}) return count(n) AS count", context);
    System.out.println("result.toString() = " + result.toString());
  }
}
